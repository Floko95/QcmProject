...
if(isset($_POST['q']) and trim($_POST['q']!=''))
{
	...
	$ligne=$req->fetch(PDO::FETCH_ASSOC);
		...
		$req2=$bdd->prepare("SELECT distinct * FROM  reponse where id_question=:idq");
	...
	echo '<form action="Questions.php" method="post"><input type="hidden" name="id" value="'.$_POST['id'].'"/><input type="submit" value="Retour"/></form>';		
}
?>
</body>
</html>
